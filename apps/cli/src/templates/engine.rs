...
use super::manifest::{TemplateCategory, TemplateManifest};
...
impl TemplateEngine {
    ...
    pub fn create_from_template(
        ...
        for file in &template.files {
            ...
            let content = fs::read_to_string(&source_path).with_context(|| {
                format!("Failed to read template file: {}", source_path.display())
            })?;
    ...
    pub fn get_template(&self, name: &str, category: TemplateCategory) -> Option<TemplateManifest> {
        match category {
            ...
            TemplateCategory::App => super::manifest::get_app_templates().get(name).cloned(),
        }
    }
