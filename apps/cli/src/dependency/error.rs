...
impl ResolutionError {
    ...
    pub fn circular_dependency(cycle: Vec<PackageId>) -> Self {
        let suggestion =
            "Consider refactoring the dependency structure to break the circular dependency. \
             You might need to extract shared functionality into a separate package."
                .to_string();
    ...
    pub fn network_error(package: PackageId, error: impl Into<String>) -> Self {
        ResolutionError::NetworkError {
            package,
            error: error.into(),
            retry_suggestion: "Check your network connection and try again. You can also try clearing the cache with 'spore cache clean'.".to_string(),
        }
    }
