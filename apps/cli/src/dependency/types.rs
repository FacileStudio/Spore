...
impl Serialize for PackageId {
    ...
    where
        S: Serializer,
    {
        let value = serde_json::to_string(&PackageIdSerde {
...
impl<'de> Deserialize<'de> for PackageId {
        ...
        D: Deserializer<'de>,
    {
        ...
        let package_id =
            serde_json::from_str::<PackageIdSerde>(&value).map_err(serde::de::Error::custom)?;
...
impl PackageVersion {
    pub fn get_applicable_dependencies(&self, context: &ResolutionContext) -> Vec<&DependencySpec> {
        ...
        deps.extend(
            self.dependencies
                .iter()
                .filter(|dep| dep.is_applicable(context)),
        );
