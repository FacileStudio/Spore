...
impl<'a> TypeScriptManager<'a> {
    ...
    fn update_existing_tsconfig(&self, tsconfig_path: &Path, alias: &str) -> Result<()> {
        ...
        let spore_packages_path = format!("{}/*", alias);
        let new_path = "./spore_packages/*";

        // Check if this path alias already exists
        if let Some(existing_paths) = paths_obj.get_mut(&spore_packages_path) {
            ...
            paths_obj.insert(spore_packages_path, json!([new_path]));
        }

        // Handle the include array - add spore_packages/**/* if not present
        let include = tsconfig_obj
            .entry("include")
        ...
        match include {
            Value::Array(arr) => {
                ...
                // Check if spore_packages is already included in any form
                if !self.has_include_pattern(arr, "spore_packages") {
                    arr.push(Value::String("spore_packages/**/*".to_string()));
                }
            }
            Value::String(single_path) => {
                // Handle case where include is a single string instead of array
                let existing_path = single_path.clone();
                *include = json!([existing_path, "spore_packages/**/*"]);
            ...
            _ => {
                ...
                *include = json!(["src/**/*", "spore_packages/**/*"]);
            }
        }
            ...
            Err(e) => {
                ...
                let fallback_tsconfig = json!({
                    "compilerOptions": {
                        ...
                        "paths": {
                            format!("{}/*", alias): ["./spore_packages/*"]
                        }
                    },
                    "include": ["src/**/*", "spore_packages/**/*"],
        ...
        println!(
            "✅ Updated tsconfig.json with alias '{}' and spore_packages include",
            alias
        );
        Ok(())
    }
    ...
    fn is_pattern_match(&self, path_str: &str, pattern: &str) -> bool {
        ...
        match pattern_base.as_str() {
            "spore_packages" => {
                // Check for various spore_packages patterns
                let spore_patterns = [
                    "spore_packages",
                    "spore_packages/*",
                    "spore_packages/**",
                ...
                spore_patterns.iter().any(|p| normalized == *p)
                    || (normalized.starts_with("spore_packages/")
                        && !normalized.starts_with("spore_packages_")
                        && normalized != "spore_packages")
            }
            "src" => {
    ...
    fn create_default_tsconfig(&self, tsconfig_path: &Path, alias: &str) -> Result<()> {
        ...
        let tsconfig = json!({
            "compilerOptions": {
                ...
                "forceConsistentCasingInFileNames": true,
                "paths": {
                    spore_packages_path: ["./spore_packages/*"]
                }
            },
            "include": ["src/**/*", "spore_packages/**/*"],
            "exclude": ["node_modules", "dist"]
        ...
        println!(
            "✅ Created tsconfig.json with alias '{}' and spore_packages include",
            alias
        );
        Ok(())
    }
