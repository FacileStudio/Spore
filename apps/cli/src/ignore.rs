...
/// Represents ignore patterns loaded from .sporeignore file
#[derive(Debug, Clone)]
pub struct SporeIgnore {
    patterns: Vec<String>,
}

impl SporeIgnore {
    /// Create a new SporeIgnore from a .sporeignore file path
    ...
    /// Create a default SporeIgnore with common ignore patterns
    pub fn default() -> Self {
        let default_patterns = vec![
            "*.tar.gz".to_string(),
            "*.tgz".to_string(),
            ".sporeignore".to_string(),
            ".git".to_string(),
            ".git/".to_string(),
    ...
    /// Parse the content of a .sporeignore file
...
mod tests {
    ...
    fn test_default_ignore() {
        let ignore = SporeIgnore::default();
    ...
    fn test_glob_patterns() {
        let mut ignore = SporeIgnore::default();
    ...
    #[test]
    fn test_sporeignore_file() -> Result<()> {
        let dir = tempdir()?;
        let ignore_file = dir.path().join(".sporeignore");
        ...
        let ignore = SporeIgnore::from_file(&ignore_file)?;
    ...
    #[test]
    fn test_missing_sporeignore_file() -> Result<()> {
        let dir = tempdir()?;
        let missing_file = dir.path().join(".sporeignore");

        let ignore = SporeIgnore::from_file(&missing_file)?;
    ...
    #[test]
    fn test_default_patterns_include_sporeignore() -> Result<()> {
        let ignore = SporeIgnore::default();

        // Test that .sporeignore itself is ignored
        assert!(ignore.is_ignored(".sporeignore"));
