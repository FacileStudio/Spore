...
type InputValidatorError = Box<dyn std::error::Error + Send + Sync>;
type InputValidator = fn(&str) -> Result<Validation, InputValidatorError>;
...
pub fn prompt_for_input_with_validation(
    ...
    custom_validator: Option<InputValidator>,
) -> Result<String> {
    ...
    if let Some(validator) = custom_validator {
        ...
        text_prompt = text_prompt.with_validator(move |input: &str| {
            if input.trim().is_empty() && is_required {
                Ok(Validation::Invalid("⚠️  This field is required".into()))
...
pub fn prompt_for_description_with_help(
    prompt: &str,
    default: Option<&str>,
    help_text: Option<&str>,
) -> Result<String> {
...
#[allow(dead_code)]
pub fn prompt_for_select_with_help(
    prompt: &str,
    options: Vec<&str>,
    help_text: Option<&str>,
) -> Result<String> {
    ...
    let help_message =
        help_text.unwrap_or("Use arrow keys or j/k to navigate, Enter to select, Esc to cancel");
...
pub fn prompt_for_confirm_with_help(
    prompt: &str,
    default: Option<bool>,
    help_text: Option<&str>,
) -> Result<bool> {
    ...
    match Project::find_project_root(current_dir) {
        Ok(project_root) => {
            ...
            let context = if project_root == *current_dir {
                ...
                format!(
                    "in Spore project, will create in {}/ directory relative to project root",
                    item_type
                )
            };
        ...
        Err(_) => {
            // Not in a Spore project, create in current directory
            let context = "outside Spore project, will create in current directory".to_string();
...
pub fn format_api_error(status: reqwest::StatusCode, response_text: &str) -> String {
    ...
    match status.as_u16() {
        ...
        401 => "🔐 Authentication failed. Please log in again with 'spore auth'".to_string(),
