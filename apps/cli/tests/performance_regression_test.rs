...
fn create_large_project(num_apps: usize, num_packages: usize) -> Result<TempDir> {
    ...
    // Create main spore.yml with many apps
    let mut spore_yml = String::from(
        r#"
...
"#,
    ...
    // Add apps to spore.yml
    for i in 0..num_apps {
        spore_yml.push_str(&format!(
            r#"
  app-{}:
    tsAlias: true
    packages:
...
fn test_startup_performance() -> Result<()> {
    ...
    let (_, _, success, duration) = run_spore_command_timed(&["--help"], project.path());
    ...
    let (_, _, success, duration) = run_spore_command_timed(&["--version"], project.path());

    if success {
        assert!(
            duration < Duration::from_secs(3),
            "Version should be very fast, took {:?}",
            duration
        );
...
fn test_status_command_performance() -> Result<()> {
    ...
    let project_sizes = vec![(2, 2, "small"), (5, 3, "medium"), (10, 5, "large")];

    for (num_apps, num_packages, size_name) in project_sizes {
        let project = create_large_project(num_apps, num_packages)?;

        let (stdout, stderr, success, duration) =
            run_spore_command_timed(&["status"], project.path());
        let output = format!("{}{}", stdout, stderr);

        // Status should complete in reasonable time
        assert!(
            duration < Duration::from_secs(30),
            "Status on {} project should complete in <30s, took {:?}",
            size_name,
            duration
        );
        ...
        if !success && !output.is_empty() {
            assert!(
                output.contains("project") || output.contains("config"),
                "Status error should be meaningful: {}",
                output
            );
        }

        println!(
            "✓ Status command on {} project completed in {:?}",
            size_name, duration
        );
    }
...
fn test_script_discovery_performance() -> Result<()> {
    ...
    let test_locations = vec![
        (project.path().to_path_buf(), "project root"),
        (project.path().join("apps").join("app-0"), "app directory"),
        (
            project.path().join("packages").join("utils-0"),
            "package directory",
        ),
    ];
    ...
    for (location, context) in test_locations {
        if location.exists() {
            ...
            let (_stdout, _stderr, success, duration) =
                run_spore_command_timed(&["run", "build"], &location);

            // Should complete quickly even with many scripts
            assert!(
                duration < Duration::from_secs(10),
                "Script discovery in {} should be fast, took {:?}",
                context,
                duration
            );
            ...
            let (stdout, stderr, _success, duration) =
                run_spore_command_timed(&["run", "nonexistent"], &location);
            let output = format!("{}{}", stdout, stderr);

            assert!(
                duration < Duration::from_secs(15),
                "Script not found handling should be fast, took {:?}",
                duration
            );

            if output.contains("Available scripts") || output.contains("not found") {
                println!(
                    "✓ Script not found handled efficiently in {} in {:?}",
                    context, duration
                );
            }
        }
...
fn test_config_parsing_performance() -> Result<()> {
    ...
    let config_tests = vec![
        (project.path().join("spore.yml"), "project config"),
        (
            project.path().join("apps").join("app-0").join("app.yml"),
            "app config",
        ),
        (
            project
    ...
    for (config_path, config_type) in config_tests {
        if config_path.exists() {
            ...
            for (command, operation) in operations {
                let (stdout, stderr, success, duration) =
                    run_spore_command_timed(&command, parent_dir);

                // Config parsing should be efficient
                assert!(
                    duration < Duration::from_secs(20),
                    "{} on {} should be fast, took {:?}",
                    operation,
                    config_type,
                    duration
                );
                ...
                if success || !format!("{}{}", stdout, stderr).is_empty() {
                    println!(
                        "✓ {} on {} completed in {:?}",
                        operation, config_type, duration
                    );
                }
            }
        }
...
fn test_memory_usage_stability() -> Result<()> {
    ...
    let repeated_operations = vec![vec!["status"], vec!["run", "build-all"], vec!["--help"]];
    ...
    for operation in repeated_operations {
        ...
        for i in 0..5 {
            let (_stdout, _stderr, success, duration) =
                run_spore_command_timed(&operation, project.path());
            ...
            if i > 0 {
                ...
                assert!(
                    current_duration < max_acceptable,
...
fn test_concurrent_safety() -> Result<()> {
    ...
    for operation in operations {
        ...
        assert!(
            ...
            "Operation {:?} should complete reasonably fast, took {:?}",
            operation,
            duration
        );
    ...
    assert!(
        ...
        "All operations should complete in reasonable total time, took {:?}",
        total_duration
    );
...
fn test_large_config_handling() -> Result<()> {
    ...
    let operations = vec![
        ...
        (
            vec!["run", "build-all"],
            "script execution with large config",
        ),
    ...
    for (command, description) in operations {
        ...
        assert!(
            ...
            "{} should handle large config efficiently, took {:?}",
            description,
            duration
        );
...
fn test_error_handling_performance() -> Result<()> {
    ...
    for (command, scenario) in error_scenarios {
        ...
        assert!(
            duration < Duration::from_secs(15),
            "Error handling for {} should be fast, took {:?}",
            scenario,
            duration
        );
...
"#;
    fs::write(temp_dir.path().join("spore.yml"), invalid_yml)?;

    let (_stdout, _stderr, _success, duration) =
        run_spore_command_timed(&["status"], temp_dir.path());

    // Invalid config error should be handled quickly
    assert!(
        duration < Duration::from_secs(10),
        "Invalid config error should be fast, took {:?}",
        duration
    );
...
fn test_file_system_performance() -> Result<()> {
    ...
    for (command, operation) in fs_operations {
        ...
        assert!(
            ...
            "{} should handle file system efficiently, took {:?}",
            operation,
            duration
        );
