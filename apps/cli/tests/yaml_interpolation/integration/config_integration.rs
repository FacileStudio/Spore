...
use crate::config::{SporeConfig, PackageConfig, AppConfig, AppDependencies, TsAlias};
use std::collections::HashMap;

/// Extended interpolator that works with real Spore config types
pub struct SporeConfigInterpolator {
    interpolator: YamlInterpolator,
}

impl SporeConfigInterpolator {
    pub fn new() -> Self {
        let mut interpolator = YamlInterpolator::new();
        // Add common built-in variables for Spore configs
        interpolator.add_built_in_var("spore_version", "1.0.0");
    ...
    pub fn parse_spore_config(&self, yaml_content: &str) -> anyhow::Result<SporeConfig> {
...
#[cfg(test)]
mod spore_config_tests {
    use super::*;

    #[test]
    fn test_spore_config_basic_interpolation() {
        let yaml_content = r#"
variables:
...
"#;
        ...
        let interpolator = SporeConfigInterpolator::new();
        let config = interpolator.parse_spore_config(yaml_content).unwrap();
    ...
    #[test]
    fn test_spore_config_with_ts_alias_interpolation() {
        let yaml_content = r#"
variables:
...
"#;
    ...
    #[test]
    fn test_spore_config_complex_script_interpolation() {
        let yaml_content = r#"
variables:
  project_name: "spore-workspace"
  build_tool: "turbo"
  test_tool: "jest"
...
"#;
        ...
        assert_eq!(scripts.get("docker-build").unwrap(), "docker build -t registry.example.com/spore-workspace:latest .");
        assert_eq!(scripts.get("docker-push").unwrap(), "docker push registry.example.com/spore-workspace:latest");
...
mod variable_inheritance_tests {
    ...
    fn test_builtin_variables_in_real_configs() {
        let yaml_content = r#"
name: ${project_name}
description: "Built with Spore v${spore_version}"

scripts:
  info: "echo 'Project: ${project_name}, Node: ${node_version}, TypeScript: ${typescript_version}'"
  version: "echo 'Spore version: ${spore_version}'"
"#;

        let interpolator = SporeConfigInterpolator::new();
        let config = interpolator.parse_spore_config(yaml_content).unwrap();

        assert_eq!(config.name, "spore-test"); // Built-in project_name
        assert_eq!(config.description, Some("Built with Spore v1.0.0".to_string()));

        let scripts = config.scripts.unwrap();
        assert_eq!(scripts.get("info").unwrap(), "echo 'Project: spore-test, Node: 18.0.0, TypeScript: 5.0.0'");
        assert_eq!(scripts.get("version").unwrap(), "echo 'Spore version: 1.0.0'");
    }
...
variables:
  ...
  spore_version: "2.0.0"               # Override built-in
...
"#;
        ...
        assert_eq!(config.description, Some("Built with Spore v2.0.0".to_string()));
