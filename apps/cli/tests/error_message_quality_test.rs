...
fn is_helpful_error_message(message: &str) -> bool {
    ...
    let has_suggestion =
        message.contains("💡") || message.contains("Try") || message.contains("Please");
    let has_context =
        message.contains("because") || message.contains("since") || message.contains("due to");
    let has_action = message.contains("Run")
    ...
    let is_descriptive =
        message.len() > 20 && !message.contains("Error") && !message.contains("Failed");
...
fn test_missing_config_file_errors() -> Result<()> {
    ...
    // Test running spore in directory with no config files
    ...
    assert!(
        error_message.contains("spore.yml") || error_message.contains("config"),
        "Error should mention config files: {}",
        error_message
    );

    assert!(
        is_helpful_error_message(&error_message),
    ...
    assert!(
        error_message.contains("💡") || error_message.contains("Run from"),
...
"#;
    ...
    if error_message.contains("version") || error_message.contains("missing") {
        assert!(
            is_helpful_error_message(&error_message),
            "Missing field error should be helpful: {}",
            error_message
        );
    }
...
"#;
    ...
    let (stdout, stderr, _success) = run_spore_command(&["run", "nonexistent"], temp_dir.path());

    let error_message = format!("{}{}", stdout, stderr);

    // Should provide helpful error about script not found
    assert!(
        error_message.contains("not found") || error_message.contains("❌"),
        "Should indicate script not found: {}",
        error_message
    );

    // Should list available scripts
    assert!(
        error_message.contains("Available scripts")
            || error_message.contains("build")
            || error_message.contains("test"),
        "Should list available scripts: {}",
        error_message
    );
    ...
    assert!(
        is_helpful_error_message(&error_message),
        "Script not found error should be helpful: {}",
        error_message
    );
...
"#;
    ...
    if error_message.contains("auth")
        || error_message.contains("login")
        || error_message.contains("🔐")
    {
        assert!(
            is_helpful_error_message(&error_message),
            "Auth error should be helpful: {}",
            error_message
        );

        // Should suggest how to authenticate
        assert!(
            error_message.contains("spore auth") || error_message.contains("log in"),
...
"#;
    fs::write(temp_dir.path().join("spore.yml"), invalid_name_yml)?;

    let (stdout, stderr, _success) = run_spore_command(&["status"], temp_dir.path());

    let error_message = format!("{}{}", stdout, stderr);

    // Should provide specific validation error
    if error_message.contains("invalid")
        || error_message.contains("path")
        || error_message.contains("characters")
    {
        assert!(
            is_helpful_error_message(&error_message),
            "Validation error should be helpful: {}",
            error_message
        );
    }
...
"#;
    ...
    if error_message.contains("package")
        || error_message.contains("dependency")
        || error_message.contains("not found")
    {
        assert!(
            is_helpful_error_message(&error_message),
            "Dependency error should be helpful: {}",
            error_message
        );
    }
...
"#;
    ...
    let (stdout, stderr, _success) = run_spore_command(&["run"], temp_dir.path());
    ...
    if error_message.contains("Interactive") || error_message.contains("terminal") {
        assert!(
            is_helpful_error_message(&error_message),
            "Interactive mode error should be helpful: {}",
            error_message
        );

        // Should suggest alternative
        assert!(
            error_message.contains("spore run <script_name>")
                || error_message.contains("specific script"),
            "Should suggest alternative: {}",
            error_message
        );
    }
...
"#;
    ...
    if error_message.contains("version") || error_message.contains("semver") {
        assert!(
            is_helpful_error_message(&error_message),
            "Version error should be helpful: {}",
            error_message
        );

        // Should explain semver format
        assert!(
            error_message.contains("x.y.z")
                || error_message.contains("semver")
                || error_message.contains("format"),
            "Should explain version format: {}",
            error_message
        );
    }
...
fn test_help_message_quality() -> Result<()> {
    ...
    // Test main help
    let (stdout, stderr, _) = run_spore_command(&["--help"], temp_dir.path());
    let help_message = format!("{}{}", stdout, stderr);

    // Help should be comprehensive and well-formatted
    assert!(
        help_message.contains("Spore") && help_message.contains("Monorepo"),
        "Help should contain project description: {}",
        help_message
    );

    assert!(
        help_message.contains("Commands:")
            || help_message.contains("COMMANDS")
            || help_message.contains("SUBCOMMANDS"),
        "Help should list commands: {}",
        help_message
    );

    // Test subcommand help
    let (stdout, stderr, _) = run_spore_command(&["run", "--help"], temp_dir.path());
    let run_help = format!("{}{}", stdout, stderr);

    assert!(
        run_help.contains("run") || run_help.contains("script"),
        "Run help should mention scripts: {}",
        run_help
    );

    // Should not contain outdated information
    assert!(
        !run_help.contains("ctrl-j") && !run_help.contains("ctrl-k"),
        "Help should not contain outdated ctrl-j/ctrl-k references: {}",
        run_help
    );
...
fn test_error_context_and_suggestions() -> Result<()> {
    ...
    for (command, context_keywords) in test_cases {
        ...
        if !output.is_empty() && has_context {
            assert!(
                is_helpful_error_message(&output),
                "Command {:?} should provide helpful output: {}",
                command,
                output
            );
        }
...
"#;
    ...
    let test_commands = vec![vec!["run", "nonexistent"], vec!["status"], vec!["link"]];
    ...
    for command in test_commands {
        ...
        if !output.is_empty() {
            ...
            if output.len() > 50 {
                assert!(
                    has_appropriate_emoji || is_helpful_error_message(&output),
                    "Command {:?} should have appropriate emoji or helpful formatting: {}",
                    command,
                    output
                );
            }
