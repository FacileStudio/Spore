...
<svelte:head>
	...
	<link rel="canonical" href="https://spore.klysium.com/" />
	...
	<script type="application/ld+json">
	{
		...
		"potentialAction": {
			"@type": "SearchAction",
			"target": "https://spore.klysium.com/packages?search={search_term_string}",
	...
	<div class="max-w-2xl mx-auto text-center">
		<div class="bg-black/5 border border-border rounded-lg p-8">
			...
			<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded mb-6 text-left overflow-x-auto relative">
				<code>curl -fsSL https://raw.githubusercontent.com/saravenpi/spore/main/install.sh | bash</code>
				<button
					class="absolute top-2 right-2 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors"
					on:click={async () => {
				try {
					await navigator.clipboard.writeText('curl -fsSL https://raw.githubusercontent.com/saravenpi/spore/main/install.sh | bash');
				} catch (err) {
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
			...
			<div class="space-y-4">
				...
				<div class="bg-muted/30 rounded-lg p-4">
					...
					<pre class="text-sm font-mono bg-black/90 text-green-400 p-3 rounded overflow-x-auto"><code>$ spore init MyProject
✅ Initialized new Spore project: MyProject
📁 Created directories: packages/, apps/
💡 Use 'spore init:package &lt;name&gt;' to create packages</code></pre>
				</div>
				<div class="bg-muted/30 rounded-lg p-4">
					<div class="text-sm text-muted-foreground mb-2">spore.yml</div>
...
<span class="text-blue-400">scripts:</span>
  ...
  <span class="text-blue-400">test-all:</span> <span class="text-green-400">"spore run test --all"</span></code></pre>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="bg-muted/30 rounded-lg p-4">
					<div class="text-sm text-muted-foreground mb-2">Terminal</div>
					<pre class="text-sm font-mono bg-black/90 text-green-400 p-3 rounded overflow-x-auto"><code>$ spore init:package utils --team myteam
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
		<div class="text-center">
			...
			<p class="text-muted-foreground">
				Publish packages instantly with the Spore CLI. Simple, secure, and integrated with your workflow.
			</p>
		</div>
		<div class="text-center">
			<div class="w-12 h-12 bg-black/10 rounded-lg flex items-center justify-center mx-auto mb-4">
