...
<svelte:head>
	<title>Quick Start Guide - Spore CLI Documentation</title>
	<meta name="description" content="Get started with Spore CLI in under 5 minutes. Learn to initialize projects, create packages, and link dependencies for TypeScript monorepos." />
	<meta property="og:title" content="Quick Start Guide - Spore CLI" />
	<meta property="og:description" content="Get started with Spore CLI in under 5 minutes. Learn to initialize projects, create packages, and link dependencies for TypeScript monorepos." />
	<meta property="og:image" content="/images/og/quick-start.png" />
	<meta property="og:url" content="https://spore.klysium.com/docs/quick-start" />
	<meta name="twitter:title" content="Quick Start Guide - Spore CLI" />
	<meta name="twitter:description" content="Get started with Spore CLI in under 5 minutes. Learn to initialize projects, create packages, and link dependencies for TypeScript monorepos." />
	<meta name="twitter:image" content="/images/og/quick-start.png" />
	<link rel="canonical" href="https://spore.klysium.com/docs/quick-start" />
</svelte:head>
...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
	<div class="mb-8">
		...
		<p class="text-lg text-muted-foreground">
			Get your first Spore project up and running in under 5 minutes
		</p>
	</div>
			...
			<div class="flex items-start space-x-3">
				...
				<div>
					...
					<ul class="text-sm text-blue-700 space-y-1">
						...
						<li>• Spore CLI installed (<a href="/docs/installation" class="underline hover:no-underline">installation guide</a>)</li>
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore init my-awesome-project</code>
					</div>
					<button 
						...
						on:click={() => copyToClipboard('spore init my-awesome-project')}
					>
						{#if showCopied && copyText === 'spore init my-awesome-project'}
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>spore init:package utils</code>
					<button 
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>spore init:app frontend</code>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
						on:click={() => copyToClipboard('spore init:app frontend')}
					>
						{#if showCopied && copyText === 'spore init:app frontend'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>cd apps/frontend && spore install utils</code>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
						on:click={() => copyToClipboard('cd apps/frontend && spore install utils')}
					>
						{#if showCopied && copyText === 'cd apps/frontend && spore install utils'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>spore run dev</code>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
						on:click={() => copyToClipboard('spore run dev')}
					>
						{#if showCopied && copyText === 'spore run dev'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div class="flex items-start space-x-3">
				...
				<div>
					...
					<p class="text-sm text-green-700 mb-4">
						You've successfully created your first Spore monorepo with a shared package and a React app that uses it.
