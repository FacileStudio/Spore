...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
	...
	<section class="mb-12">
		...
		<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
			<div class="overflow-x-auto p-4 pr-12">
				<code class="whitespace-nowrap block">{@html `<span class="text-green-400">git</span> <span class="text-blue-400">clone</span> <span class="text-purple-400">https://github.com/saravenpi/spore.git</span>`}</code>
			</div>
			<button 
				...
				on:click={() => copyToClipboard('git clone https://github.com/saravenpi/spore.git')}
			>
				{#if showCopied && copyText === 'git clone https://github.com/saravenpi/spore.git'}
	...
	<section class="mb-12">
		...
		<p class="text-muted-foreground mb-3">
			Navigate to the `apps/spore-space` directory and create a `.env` file from the example.
		</p>
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>{@html `<span class="text-green-400">docker</span> <span class="text-blue-400">build</span> <span class="text-yellow-400">-t</span> <span class="text-purple-400">spore-space</span> <span class="text-white">.</span>`}</code>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
						on:click={() => copyToClipboard('docker build -t spore-space .')}
					>
						{#if showCopied && copyText === 'docker build -t spore-space .'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm p-4 rounded-lg relative group">
					<code>{@html `<span class="text-green-400">docker</span> <span class="text-blue-400">run</span> <span class="text-yellow-400">-d</span> <span class="text-yellow-400">-p</span> <span class="text-purple-400">3000:3000</span> <span class="text-yellow-400">--env-file</span> <span class="text-purple-400">.env</span> <span class="text-purple-400">spore-space</span>`}</code>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
						on:click={() => copyToClipboard('docker run -d -p 3000:3000 --env-file .env spore-space')}
					>
						{#if showCopied && copyText === 'docker run -d -p 3000:3000 --env-file .env spore-space'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
