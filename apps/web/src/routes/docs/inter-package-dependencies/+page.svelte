...
<svelte:head>
	...
	<meta property="og:url" content="https://spore.klysium.com/docs/inter-package-dependencies" />
	...
	<link rel="canonical" href="https://spore.klysium.com/docs/inter-package-dependencies" />
</svelte:head>
...
<span class="text-blue-400">dependencies:</span>
  ...
  <span class="text-gray-400"># Remote Spore packages</span>
				...
				<div class="space-y-4">
					<div>
						...
						<div class="bg-black/90 text-green-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps add types --app my-app</code>
							<button 
								...
								on:click={() => copyToClipboard('spore deps add types --app my-app')}
							>
								{#if showCopied && copyText === 'spore deps add types --app my-app'}
					...
					<div>
						...
						<div class="bg-black/90 text-green-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps add "@team/design-system@^2.0.0" --app frontend</code>
							<button 
								class="absolute top-1 right-1 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
								on:click={() => copyToClipboard('spore deps add "@team/design-system@^2.0.0" --app frontend')}
							>
								{#if showCopied && copyText === 'spore deps add "@team/design-system@^2.0.0" --app frontend'}
									<Icon icon="lucide:check-circle" class="w-3 h-3 text-green-400" />
								{:else}
									<Icon icon="lucide:copy" class="w-3 h-3" />
								{/if}
					...
					<div>
						...
						<div class="bg-black/90 text-green-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps add test-utils --dev --app my-package</code>
							<button 
								class="absolute top-1 right-1 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
								on:click={() => copyToClipboard('spore deps add test-utils --dev --app my-package')}
							>
								{#if showCopied && copyText === 'spore deps add test-utils --dev --app my-package'}
									<Icon icon="lucide:check-circle" class="w-3 h-3 text-green-400" />
								{:else}
									<Icon icon="lucide:copy" class="w-3 h-3" />
								{/if}
				...
				<div class="space-y-4">
					<div>
						...
						<div class="bg-black/90 text-blue-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps list --app frontend</code>
							<button 
								class="absolute top-1 right-1 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
								on:click={() => copyToClipboard('spore deps list --app frontend')}
							>
								{#if showCopied && copyText === 'spore deps list --app frontend'}
									<Icon icon="lucide:check-circle" class="w-3 h-3 text-green-400" />
								{:else}
									<Icon icon="lucide:copy" class="w-3 h-3" />
								{/if}
					...
					<div>
						...
						<div class="bg-black/90 text-blue-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps tree --app backend --depth 3</code>
							<button 
								class="absolute top-1 right-1 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
								on:click={() => copyToClipboard('spore deps tree --app backend --depth 3')}
							>
								{#if showCopied && copyText === 'spore deps tree --app backend --depth 3'}
									<Icon icon="lucide:check-circle" class="w-3 h-3 text-green-400" />
								{:else}
									<Icon icon="lucide:copy" class="w-3 h-3" />
								{/if}
					...
					<div>
						...
						<div class="bg-black/90 text-yellow-400 font-mono text-sm p-3 rounded-lg relative group">
							<code>spore deps check</code>
							<button 
								class="absolute top-1 right-1 p-1.5 rounded bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100"
								on:click={() => copyToClipboard('spore deps check')}
							>
								{#if showCopied && copyText === 'spore deps check'}
									<Icon icon="lucide:check-circle" class="w-3 h-3 text-green-400" />
								{:else}
									<Icon icon="lucide:copy" class="w-3 h-3" />
								{/if}
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<div class="border border-red-200 bg-red-50 rounded-lg p-6">
					...
					<div class="space-y-4 text-sm">
						...
						<div>
							...
							<div class="text-red-600 mt-1"><strong>Solution:</strong> Use <code>spore deps sync</code> to align versions</div>
						</div>
						<div>
				...
				<div class="border border-blue-200 bg-blue-50 rounded-lg p-6">
					...
					<div class="space-y-3 text-sm">
						<div>
							<div class="font-medium text-blue-800"><code>spore deps why package-name</code></div>
							<div class="text-blue-600">Explain why a package is included</div>
						</div>
						<div>
							<div class="font-medium text-blue-800"><code>spore deps resolve --dry-run</code></div>
							<div class="text-blue-600">Preview dependency resolution without changes</div>
						</div>
						<div>
							<div class="font-medium text-blue-800"><code>spore link --verbose</code></div>
							<div class="text-blue-600">Show detailed linking information</div>
						</div>
						<div>
							<div class="font-medium text-blue-800"><code>spore deps outdated</code></div>
...
spore deps resolve --strategy latest --app affected-app</code>
						</div>
					</div>
