...
<svelte:head>
	...
	<meta property="og:url" content="https://spore.klysium.com/docs/project-management" />
	...
	<link rel="canonical" href="https://spore.klysium.com/docs/project-management" />
</svelte:head>
...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
	...
	<section class="mb-12">
		...
		<p class="text-muted-foreground mb-6 leading-relaxed">
			Start a new monorepo with a single command. Spore will set up the basic structure for you, including the root <code class="bg-muted px-1.5 py-0.5 rounded text-sm">spore.yml</code> configuration file and organize your workspace.
		</p>
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore init &lt;project-name&gt;</code>
					</div>
					<button 
						...
						on:click={() => copyToClipboard('spore init my-project')}
					>
						{#if showCopied && copyText === 'spore init my-project'}
...
└── <span class="text-gray-400">README.md</span>         # Project documentation</code></pre>
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore init .</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore init .')}
					>
						{#if showCopied && copyText === 'spore init .'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
	...
	<section class="mb-12">
		...
		<p class="text-muted-foreground mb-6 leading-relaxed">
			The <code class="bg-muted px-1.5 py-0.5 rounded text-sm">spore.yml</code> file is the heart of your Spore project. It defines the project's metadata, scripts, and global configurations that apply to your entire monorepo.
		</p>
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-white font-mono text-sm p-4 rounded-lg">
					<pre><code>{@html `<span class="text-gray-400"># spore.yml - Project configuration</span>
...
<span class="text-blue-400">scripts:</span>
  ...
  <span class="text-blue-400">build:</span> <span class="text-green-400">"spore run build --all"</span>
  <span class="text-blue-400">dev:</span> <span class="text-green-400">"spore run dev --parallel"</span>`}</code></pre>
				</div>
				<p class="text-sm text-muted-foreground mt-2">
			...
			<div>
				...
				<div class="bg-black/90 text-white font-mono text-sm p-4 rounded-lg">
					<pre><code>{@html `<span class="text-gray-400"># Advanced spore.yml configuration</span>
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore status</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore status')}
					>
						{#if showCopied && copyText === 'spore status'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore status --detailed</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore status --detailed')}
					>
						{#if showCopied && copyText === 'spore status --detailed'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore run &lt;script-name&gt;</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore run build')}
					>
						{#if showCopied && copyText === 'spore run build'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
				...
				<p class="text-sm text-muted-foreground mt-2">
					Executes the specified script defined in your <code>spore.yml</code> file.
				</p>
			</div>
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore run build --parallel</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore run build --parallel')}
					>
						{#if showCopied && copyText === 'spore run build --parallel'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore run test --filter="packages/*"</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore run test --filter="packages/*"')}
					>
						{#if showCopied && copyText === 'spore run test --filter="packages/*"'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
	...
	<section class="mb-12">
		...
		<div class="space-y-6">
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore clean</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore clean')}
					>
						{#if showCopied && copyText === 'spore clean'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore update</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore update')}
					>
						{#if showCopied && copyText === 'spore update'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
			...
			<div>
				...
				<div class="bg-black/90 text-green-400 font-mono text-sm rounded-lg relative group">
					<div class="overflow-x-auto p-4 pr-12">
						<code class="whitespace-nowrap block">spore validate</code>
					</div>
					<button 
						class="absolute top-2 right-2 p-2 rounded-md bg-white/10 hover:bg-white/20 transition-colors opacity-0 group-hover:opacity-100 z-10"
						on:click={() => copyToClipboard('spore validate')}
					>
						{#if showCopied && copyText === 'spore validate'}
							<Icon icon="lucide:check-circle" class="w-4 h-4 text-green-400" />
						{:else}
							<Icon icon="lucide:copy" class="w-4 h-4" />
						{/if}
				...
				<div class="flex items-start space-x-3">
					...
					<div>
						...
						<ul class="text-sm text-green-700 space-y-1">
							...
							<li>• Regularly run <code>spore status</code> to monitor health</li>
						</ul>
					</div>
				</div>
			</div>
			...
			<div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
				<div class="flex items-start space-x-3">
					...
					<div>
						...
						<ul class="text-sm text-blue-700 space-y-1">
							<li>• Version your <code>spore.yml</code> configuration</li>
				...
				<div class="flex items-start space-x-3">
					...
					<div>
						...
						<ul class="text-sm text-orange-700 space-y-1">
							<li>• Run <code>spore clean</code> before major builds</li>
