<script lang="ts">
	...
	onMount(async () => {
		...
		if (browser) {
			token = localStorage.getItem('spore_token') || '';
		}
		pageLoading = false;
	});
		...
		try {
			...
			if (response.ok && result.success) {
				...
				if (result.data.token) {
					localStorage.setItem('spore_token', result.data.token);
...
<div class="max-w-4xl mx-auto">
	<div class="mb-8">
		<h1 class="text-3xl font-bold tracking-tight" style="font-family: 'Goga', 'Satoshi', sans-serif;">Settings</h1>
	...
	<div class="space-y-6">
		...
		<div class="bg-card text-card-foreground rounded-lg border p-6">
			...
			<p class="text-muted-foreground mb-6">
				Use this token to authenticate with the Spore CLI for publishing packages and managing your account.
			</p>
			...
			<div class="space-y-4">
				...
				<div class="bg-muted/50 rounded-md p-4">
					...
					<div class="space-y-2 text-sm text-muted-foreground">
						<p><strong>1. Install the Spore CLI:</strong></p>
