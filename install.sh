...
check_existing_installation() {
    if command_exists spore; then
        CURRENT_VERSION=$(spore --version 2>/dev/null | sed 's/spore //' || echo "unknown")
        INSTALL_PATH=$(which spore)
        IS_UPDATE=true
        print_step "Found existing Spore installation"
        print_step "Current version: $CURRENT_VERSION"
        print_step "Location: $INSTALL_PATH"
    else
        IS_UPDATE=false
        print_step "No existing Spore installation found"
    fi
}
...
get_latest_version() {
    ...
    LATEST_VERSION=$(curl -s https://api.github.com/repos/saravenpi/spore/releases/latest | grep '"tag_name"' | sed 's/.*"tag_name": "\([^"]*\)".*/\1/' | sed 's/^v//')
...
use_cached_source() {
    CACHE_DIR="$HOME/.spore-build-cache"
...
# Clone repository
clone_repository() {
    print_step "Cloning Spore repository..."

    REPO_URL="${SPORE_REPO_URL:-https://github.com/saravenpi/spore.git}"
...
build_project() {
    ...
    if use_cached_source; then
        src_dir="$HOME/.spore-build-cache/src"
        use_cache=true
        print_step "Using cached source for faster build"
    else
        src_dir="$TEMP_DIR/spore"
    ...
    if [ "$use_cache" = true ]; then
        export CARGO_TARGET_DIR="$HOME/.spore-build-cache/target"
    ...
    if ! cargo build --release; then
        print_error "Failed to build Spore"
        exit 1
    fi

    # Set binary path based on build location
    if [ "$use_cache" = true ]; then
        BINARY_PATH="$HOME/.spore-build-cache/target/release/spore"
    else
        BINARY_PATH="$src_dir/apps/cli/target/release/spore"
    fi
...
# Install binary
install_binary() {
    print_step "Installing Spore binary..."
    ...
    if [ "$IS_UPDATE" = true ]; then
        print_success "Spore updated successfully at $TARGET_PATH"
    else
        print_success "Spore installed to $TARGET_PATH"
    fi
}
...
verify_installation() {
    ...
    if command_exists spore; then
        VERSION=$(spore --version 2>/dev/null || echo "spore 0.1.0")
        print_success "Spore is installed and working!"
        print_success "$VERSION"
        print_success "Location: $(which spore)"
    else
        print_warning "Spore binary installed but not found in PATH"
...
show_usage() {
    echo ""
    print_step "Getting started with Spore:"
    echo "  spore init my-project          # Initialize a new project"
    echo "  spore init:package my-package  # Create a new package"
    echo "  spore init:app my-app          # Create a new app"
    echo "  spore link                     # Link packages to apps"
    echo "  spore status                   # Show project status"
    echo "  spore --help                   # Show help information"
    echo ""
    print_step "Documentation: https://github.com/saravenpi/spore"
}
...
main() {
    ...
    if [ "$IS_UPDATE" = true ]; then
        ...
        NEW_VERSION=$(spore --version 2>/dev/null | sed 's/spore //' || echo "unknown")
